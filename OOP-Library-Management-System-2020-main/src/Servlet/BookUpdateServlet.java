	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    //saving the file nname and the path
		  ...
		  boolean isTrue;
			...
			isTrue = BookDBUtil.updateBook( id ,categoryNum, bookname,bookauthor,edition,price, year, fileName , savepath);
			
			if(isTrue == true) {
				//If the db connection is a success and the data is been inserted DB
				List<Book> listBook = BookDBUtil.getBookList();
				request.setAttribute("listBook", listBook);
				RequestDispatcher dis = request.getRequestDispatcher("BookUpdateSuccess.jsp");
